...
class Product extends MY_Controller{
    ...
    private function generateBrandSelection($brands, $brands_selected){
    	...
    	foreach($brands as $brand){
    		$result = $result.'<input class="chkBrand" type="checkbox" name="brand[]" value="'.$brand['brand'].'"'.((in_array($brand['brand'], $brands_selected))?(' checked="true"'):('')).'> '.'<a href="/store/brands/'.url_title($brand['brand']).'" target="_blank" style="text-decoration: none; color:black;">'.$brand['brand'].'</a><br>'."\n";
    	}
    	return $result;
    }
